...
export function HandleHistory({ handleHistory }) {
  ...
  /** @type {string | undefined} */
  let lastPds;
  const entries = handleHistory
    .slice()
    .reverse()
    .map(([handle, date, pds], i) => {
...
function ChangeEntry({ date, handle, pds, lastHandle, lastPds, isCreated }) {
  ...
  if (!isHandleChanged && !isPdsChanged)
    return (
      <div className="event-entry">
        {date ? (
          <FormatTimestamp className="event-timestamp" timestamp={date} />
...
function AtAndHandle({ className, shortHandle }) {
  const handleAt =
    shortenHandle(shortHandle).indexOf('.') < 0 ? (
      <span className="handle-at-sign">@</span>
    ) : (
      <img
        className="handle-domain-icon"
        src={'https://icon.horse/icon/' + unwrapShortHandle(shortHandle)}
      />
    );
  return (
